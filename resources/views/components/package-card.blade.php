<div
    class="bg-white  shadow-lg px-8 py-14 w-full md:w-2/3 lg:w-1/3 text-center hover:shadow-2xl transition-all duration-300">
    <h3 class="text-2xl font-semibold mb-4">Class drop-in</h3>
    <div class="mb-6">
        <h2 class="text-5xl font-bold text-[#f36100]">$39.0</h2>
        <span class="text-gray-500 text-sm uppercase">Single Class</span>
    </div>
    <ul class="text-gray-600 space-y-2 mb-8">
        <li>Free riding</li>
        <li>Unlimited equipments</li>
        <li>Personal trainer</li>
        <li>Weight losing classes</li>
        <li>Month to month</li>
        <li>No time restriction</li>
    </ul>
    <a href="#"
       class="inline-block bg-[#f36100] text-white px-6 py-3 rounded-full font-medium hover:bg-orange-700 transition-all duration-300">
        Enroll now
    </a>
    <a href="#"
       class="inline-block mt-4 text-gray-500 hover:text-[#f36100] transition-all duration-300 text-lg">
        <i class="fa fa-picture-o"></i>
    </a>
</div>
